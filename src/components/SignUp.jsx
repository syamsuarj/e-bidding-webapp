...
const SignUp = () => {
    ...
    return (
      ...
      >
        <div className="space-y-1 sm:max-w-[60%]">
          ...
          <span
            className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${optional ? 'bg-slate-200 text-slate-700' : 'bg-primary/15 text-primary'}`}
          >
  ...
  const renderReceiptCard = (receipt) => (
    <div
      ...
      role="status"
    >
      <h3 className="text-xl font-semibold text-primary">Pengajuan berhasil diterima ✅</h3>
      <p className="mt-3 text-sm text-slate-700">
        Terima kasih, {receipt.applicant}. Permohonan pendaftaran untuk {receipt.company} telah kami terima pada{' '}
    ...
    <main className="min-h-screen bg-gradient-to-b from-primary/8 via-background to-background pb-24">
      <header className="border-b border-primary/20 bg-primary/10 py-6">
        <div className="container flex flex-wrap items-center justify-between gap-4 text-sm text-slate-700">
          <a href="/" className="inline-flex items-center gap-2 font-semibold text-primary transition hover:underline">
      ...
      <div className="container space-y-10 py-12">
        <section className="grid gap-6 rounded-3xl border border-primary/15 bg-surface p-8 shadow-soft">
          <div className="space-y-3">
            <h1 className="text-3xl font-bold text-slate-900">Form Registrasi Vendor e-Bidding APAS</h1>
          ...
          <ul className="grid gap-3 rounded-2xl bg-primary/5 p-5 text-sm text-slate-700 sm:grid-cols-3" aria-label="Persyaratan dokumen">
            ...
            <li className="rounded-xl border border-primary/10 bg-white/70 px-4 py-3">Review kelengkapan dilakukan manual oleh tim bisnis</li>
            <li className="rounded-xl border border-primary/10 bg-white/70 px-4 py-3">Pemberitahuan revisi akan dikirim otomatis via email</li>
          </ul>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,380px)]">
          <form className="space-y-8 rounded-3xl border border-primary/15 bg-surface p-8 shadow-soft" onSubmit={handleSubmit} noValidate>
            <section className="space-y-4">
              <h3 className="text-xl font-semibold text-slate-900">Data Pengguna</h3>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  ...
                  />
                  {errors.fullName && <small className={errorClasses}>{errors.fullName}</small>}
                </label>
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  ...
                  <input
                    ...
                    className={inputClasses}
                  />
                  {errors.email && <small className={errorClasses}>{errors.email}</small>}
                </label>
                <label className="grid gap-2 text-sm font-medium text-slate-700 md:col-span-2">
                  ...
                  <input
                    ...
                    className={inputClasses}
                  />
                  {errors.phone && <small className={errorClasses}>{errors.phone}</small>}
                </label>
              </div>
            </section>

            <section className="space-y-4">
              <h3 className="text-xl font-semibold text-slate-900">Data Perusahaan</h3>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  ...
                  <input
                    ...
                    className={inputClasses}
                  />
                  {errors.companyName && <small className={errorClasses}>{errors.companyName}</small>}
                </label>
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  ...
                  {errors.companyType && <small className={errorClasses}>{errors.companyType}</small>}
                </label>
              </div>
              <label className="grid gap-2 text-sm font-medium text-slate-700">
                ...
                />
                {errors.companyAddress && <small className={errorClasses}>{errors.companyAddress}</small>}
              </label>
              <label className="grid gap-2 text-sm font-medium text-slate-700">
            ...
            <section className="space-y-4">
              <h3 className="text-xl font-semibold text-slate-900">Keamanan Akun</h3>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  <span>Password</span>
                  <input
                    ...
                    className={inputClasses}
                  />
                  {errors.password && <small className={errorClasses}>{errors.password}</small>}
                </label>
                <label className="grid gap-2 text-sm font-medium text-slate-700">
                  ...
                  <input
                    ...
                    className={inputClasses}
                  />
                  {errors.confirmPassword && <small className={errorClasses}>{errors.confirmPassword}</small>}
                </label>
              </div>
            </section>

            <section className="space-y-3">
              <h3 className="text-xl font-semibold text-slate-900">Verifikasi Captcha</h3>
              <div className="flex flex-wrap items-center gap-3 rounded-xl bg-primary/10 px-4 py-3 text-sm font-semibold text-primary">
                ...
                />
                <button type="button" className="text-sm font-semibold text-primary underline" onClick={() => setCaptcha(generateCaptcha())}>
                  Ganti Soal
                </button>
              </div>
              {errors.captchaAnswer && <small className={errorClasses}>{errors.captchaAnswer}</small>}
            </section>
            ...
            <section className="space-y-3">
              <label className="flex items-start gap-3 rounded-xl bg-background/80 p-4 text-sm text-slate-700">
                ...
                <span>
                  ...
                  <button type="button" onClick={handleShowTerms} className="font-semibold text-primary underline">
                    Syarat & Ketentuan APAS
                  </button>
                </span>
              </label>
              {errors.acceptTerms && <small className={errorClasses}>{errors.acceptTerms}</small>}
            </section>
          ...
          <aside className="space-y-6 rounded-3xl border border-primary/15 bg-surface p-6 shadow-soft lg:sticky lg:top-28" aria-label="Unggah dokumen">
            <header className="space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <h3 className="text-xl font-semibold text-slate-900">Unggah Dokumen Legal</h3>
                  <p className="text-sm text-slate-600">Format diizinkan: PDF, JPG, PNG · Maks. 10 MB per file.</p>
                </div>
                <div className="rounded-xl bg-primary/10 px-4 py-2 text-right">
                  <span className="block text-lg font-bold text-primary">
                    {uploadedRequiredCount}/{requiredDocuments.length}
                  </span>
                  <small className="text-xs font-semibold uppercase tracking-wide text-primary/80">Dokumen wajib</small>
            ...
            <div className="space-y-3" aria-label="Dokumen wajib">
              <h4 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Dokumen Wajib</h4>
              <ul className="grid gap-3">
                {requiredDocuments.map((doc) => renderDocumentRow(doc, false))}
              </ul>
            </div>
            {optionalDocuments.length > 0 && (
              <details className="rounded-xl border border-primary/15 bg-background/70 p-4 text-sm text-slate-700" aria-label="Dokumen opsional">
        ...
        <section className="rounded-3xl border border-primary/15 bg-surface p-8 shadow-soft" aria-label="Informasi login">
          <h3 className="text-xl font-semibold text-slate-900">Sudah memiliki akun?</h3>
          <p className="mt-3 text-sm text-slate-600">
            Akses dashboard e-Bidding melalui portal internal APAS. Jika Anda mengalami kendala login, hubungi{' '}
            <a href="mailto:support@apas.id" className="font-semibold text-primary hover:underline">
              support@apas.id
            </a>{' '}
            untuk reset kredensial.
          </p>
          <p className="mt-2 text-xs text-slate-500">Modul login akan tersinkron dengan SSO internal pada saat peluncuran aplikasi.</p>
        </section>
      </div>

      {showTerms && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8" role="dialog" aria-modal="true" aria-labelledby="terms-title">
          <div className="absolute inset-0 bg-text/60 backdrop-blur-sm" onClick={handleHideTerms} />
          <div className="relative w-full max-w-2xl space-y-6 rounded-[28px] border border-primary/20 bg-surface p-8 shadow-2xl">
            <header className="flex items-start justify-between gap-4">
              <h3 id="terms-title" className="text-2xl font-bold text-slate-900">
